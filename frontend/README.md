...
本文档只负责前端的启动方式、页面数据流、前端问题和开发顺序。项目整体进度和每日开发记录见根目录 [README.md](../README.md)。
...
> 当前仅支持粘贴 JD 文本；URL 抓取尚未实现，后端 URL 字段仅作为未来扩展预留。
...
- 已统一 Step1 mock 与组件目标字段；
- 已移除真实分析失败时的静默 mock 降级；
- 已修复 `vite.config.js` 的 ESM lint 错误；
- Step2、Step3 仍等待后端接入真实 Dify 输出；
- 简历文本保存/读取已实现，但当前 Step2/Step3 不能进行真实的个性化分析。

### 2026-09-10 简历功能进展

- 已新增 `resume_versions` 表及 `/api/resumes`、`/api/resumes/latest` 接口；
- 前端输入页可以读取最近一版简历、编辑名称和内容，并保存新的简历版本；
- 当前简历还没有传入 Dify，下一步再接入 JD + 简历的真实 Step2/Step3 分析。

项目级每日困难和决策记录见根目录 [README.md](../README.md)。
